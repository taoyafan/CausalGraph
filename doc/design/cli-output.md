...
## 5. `outline`：全图结构鸟瞰（Agent 建立结构认知，免读源文件）

> 目的：让 Agent 不打开任何 `data/*.json` 就能一眼看清「有哪些图簇、每簇的最终产出是什么、源/中/终各多少」，固定格式、每次可复现，省上下文。**不求值、只读拓扑**。

- 三层判定纯结构、与 id 命名无关：**源**=零依赖数据节点（`DataNode`）；**中**=被其它算子 `inputs` 引用的算子节点；**终**=无人引用的算子节点（该图簇最终产出）。
- 分组＝弱连通分量（互不相连的子图各成一簇），组名复用 `webexport.GROUP_LABELS` 的中文；`★` 标该簇头条（汇聚上游最多的终点），`↑N` 为该节点传递上游节点数（越大越聚合）。
- 用法：`outline` 全图；`outline --group ind`（或 `--group 产业环节`）只看一簇；`--ops` 展开中间算子、`--data` 展开数据源（默认只给个数以省上下文）。
- 头部一行给全局计数：`总节点 / 数据源 / 中间算子 / 终点 / 分组数 / 弱连通分量数`。
