...
⑦ **值来源零计算（堵图外烘焙）**：逐个核对节点值是否 ①独立披露 或 ②直接拍定的先验；
   若 quote/note 出现『本节点值 = 对其它数字的算术结果』（除/乘/加减）且该值非独立披露 → 判为图外计算烘焙，
   打回，要求改成图内算子。assumption 的分布必须是直接设定的先验区间/众数，不得是某公式的解。
